...
ImageTexture::ImageTexture( ImageTextureInfo_t info )
{
	...
	root.m_renderContext->CreateImageTexture( info, &m_handle );
}

void ImageTexture::SetData( TextureData_t textureData )
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->SetImageTextureData( m_handle, textureData );
}

void ImageTexture::Copy( TextureCopyData_t copyData )
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->CopyImageTexture( m_handle, copyData );
}
...
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->CreateBuffer( info, &m_handle );
}

void BaseBuffer::Upload( BufferUploadInfo_t uploadInfo )
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->UploadBuffer( m_handle, uploadInfo );
}
...
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->CreateVertexBuffer( info, &m_handle );
}

IndexBuffer::IndexBuffer( BufferInfo_t info )
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->CreateIndexBuffer( info, &m_handle );
}
...
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->CreateRenderTexture( info, &m_handle );
}
...
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->CreateDescriptor( info, &m_handle );
}
...
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->CreatePipeline( info, &m_handle );
}
...
{
	auto& root = ClientRoot::GetInstance();
	root.m_renderContext->CreateShader( info, &m_handle );
}
