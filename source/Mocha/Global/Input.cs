...
public static partial class Input
{
	...
	public static unsafe void Update()
	{
		...
		else
		{
			...
			InputSnapshot = new();
			InputSnapshot.KeysDown = lastKeysDown;
			InputSnapshot.LastKeysDown = lastKeysDown;

			Sdl2Native.SDL_SetRelativeMouseMode( true );
			Sdl2Native.SDL_PumpEvents();

			List<Veldrid.KeyEvent> veldridKeyEvents = new();
			List<Veldrid.MouseEvent> veldridMouseEvents = new();
			...
			Log.Trace( $"{lastKeysDown.Count} vs {InputSnapshot.KeysDown.Count}" );

			var newKeys = InputSnapshot.KeysDown.Where( x => !lastKeysDown.Contains( x ) ).ToArray();
			foreach ( var key in newKeys )
			{
				Log.Trace( $"Key {key} was pressed" );
			}
		}
	}
