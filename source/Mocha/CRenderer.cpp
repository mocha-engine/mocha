...
void CRenderer::InitShaderResources()
{
	...
	ASSERT( mDevice->CreateDescriptorHeap( &srvHeapDesc, IID_PPV_ARGS( &mSrvHeap ) ) );
}
...
void CRenderer::SetupSwapchain( unsigned width, unsigned height )
{
	...
	else
	{
		...
		ASSERT( mFactory->CreateSwapChainForHwnd(
		    mCommandQueue, mWindow->GetWindowHandle(), &swapchainDesc, nullptr, nullptr, &swapchain ) );
...
ID3D12DescriptorHeap* CRenderer::GetSRVHeap()
{
	return mSrvHeap;
}
