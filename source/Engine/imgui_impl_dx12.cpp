...
static void ImGui_ImplDX12_SetupRenderState(
    ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx, ImGui_ImplDX12_RenderBuffers* fr )
{
...
void ImGui_ImplDX12_Shutdown()
{
	...
	{
		// We could just call ImGui_ImplDX12_DestroyWindow(main_viewport) as a convenience but that would be misleading since we
		// only use data->Resources[]
...
static void ImGui_ImplDX12_CreateWindow( ImGuiViewport* viewport )
{
	...
	// SDL_Window*). Some backends will leave PlatformHandleRaw NULL, in which case we assume PlatformHandle will contain the
	// HWND.
	...
	for ( UINT i = 0; i < bd->numFramesInFlight; ++i )
	{
		res = bd->pd3dDevice->CreateCommandAllocator(
		    D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS( &vd->FrameCtx[i].CommandAllocator ) );
		IM_ASSERT( res == S_OK );
	}

	// Create command list.
	res = bd->pd3dDevice->CreateCommandList(
	    0, D3D12_COMMAND_LIST_TYPE_DIRECT, vd->FrameCtx[0].CommandAllocator, NULL, IID_PPV_ARGS( &vd->CommandList ) );
	IM_ASSERT( res == S_OK );
...
{
	// The main viewport (owned by the application) will always have RendererUserData == NULL since we didn't create the data
	// for it.
	ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
...
static void ImGui_ImplDX12_RenderWindow( ImGuiViewport* viewport, void* )
{
	...
	if ( !( viewport->Flags & ImGuiViewportFlags_NoRendererClear ) )
		cmd_list->ClearRenderTargetView(
		    vd->FrameCtx[back_buffer_idx].RenderTargetCpuDescriptors, ( float* )&clear_color, 0, NULL );
